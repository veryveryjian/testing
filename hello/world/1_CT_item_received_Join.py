...
# Define file paths
ny_invoice_file = "C:\\Users\\charlton\\Desktop\\ct_invoice_data.xlsx"
...
po_nj_data = pd.read_excel(po_nj_file, sheet_name='Chunghua - CT', usecols=['Vendor', 'Date', 'Num', 'Amount'])
po_nj_data['Num'] = po_nj_data['Num'].astype(str)

# Perform similar conversions for CT and PA data
ny_invoice_data_ct = pd.read_excel(ny_invoice_file, sheet_name='Chung Hua Cabinet NJ', usecols=['Type', 'Date', 'Num', 'Debit'])
ny_invoice_data_ct['Num'] = ny_invoice_data_ct['Num'].astype(str)

po_ct_data = pd.read_excel(po_ny_file, sheet_name='CHUNG HUA CABINET CT INC', usecols=['Vendor', 'Date', 'Num', 'Amount'])
...
po_pa_data = pd.read_excel(po_pa_file, sheet_name='newCT', usecols=['Vendor', 'Date', 'Num', 'Amount'])
