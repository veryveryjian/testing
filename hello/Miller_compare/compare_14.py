...
# 출력 파일 이름 동적 생성
desktop_path = f'C:\\Users\\charlton\\Desktop\\{inv_filename}_{ir_filename}.xlsx'
...
with pd.ExcelWriter(desktop_path, engine='openpyxl', mode='w') as writer:
    ...
    summary_data = pd.DataFrame({
        'Sheet': ['INV', 'IR'],
        'Item Count': [df_inv_selected['Item'].count(), df_ir_selected['Item'].count()],
        ...
        'Deduplicated Item Count': [len(inv_pivot), len(ir_pivot)],
        'Pivot Qty Sum': [inv_pivot['Qty'].sum(), ir_pivot['Qty'].sum()]
    })
