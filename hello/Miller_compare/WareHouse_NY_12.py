...
if column_name in merged_df.columns:
    # 입력된 JIN# 열의 값과 CBM 값을 곱하여 Total_CBM 계산
    merged_df['Total_CBM'] = merged_df[column_name] * merged_df['CBM']

    # 총합 계산 및 출력
    total_sum_cbm = merged_df['Total_CBM'].sum()
    print(f'Total CBM for JIN#{jin_number}: {total_sum_cbm}')

    # 추가: po_ms 엑셀 파일의 JIN# 열 숫자 합계 계산 및 출력
    total_sum_jin = ny_test_df[column_name].sum()
    print(f'Total sum of numbers in JIN#{jin_number}-MS column: {total_sum_jin}')

    # 결과 데이터프레임 준비
    result_df = merged_df[['Item', column_name, 'CBM', 'Total_CBM']]

    # 엑셀 파일로 저장
    output_file_path = f'C:/Users/charlton/Desktop/C333333alculation_Result_JIN#{jin_number}.xlsx'
