...
with pd.ExcelWriter(output_file_path, engine='xlsxwriter') as writer:
    ...
    df4.to_excel(writer, sheet_name='Further_Cleaned', index=False)
    df5.to_excel(writer, sheet_name='Further_Cleaned_2', index=False)
    df6.to_excel(writer, sheet_name='Final_Prefix', index=False)
    df7.to_excel(writer, sheet_name='DF7', index=False)
    df8.to_excel(writer, sheet_name='DF8', index=False)
    df9.to_excel(writer, sheet_name='DF9', index=False)  # 새로 추가된 시트
